...
public class Transformations {
  ...
  public float b_flat(float y, float A, float B, float C){
    float tmp1 = Math.min(0, (float)Math.floor(y - B))* A*(B-y)/B;
    float tmp2 = Math.min(0, (float)Math.floor(C - y))* (1 - A)*(y - C)/(1 - C);
  ...
  public float s_decept(float y, float A, float B, float C){
    ...
    return correct_to_01(1 + tmp * (tmp1 + tmp2 + (float)1.0/B));
  } // s_decept
