      ...
      case 12:
        {
          ...
          // END minimizing structure total weight ------------------------
            ...
            fx[1] = 0;
            for (double[] element : nodeCheck_) {
                double xn=DisplacementNodes_[numberOfLibertyDegree_ * (int)element[0]+aX_][hi];
                double yn=DisplacementNodes_[numberOfLibertyDegree_ * (int)element[0]+aY_][hi];
                double zn=DisplacementNodes_[numberOfLibertyDegree_ * (int)element[0]+aZ_][hi];
...
*/
    ...
    for(int hi=0; hi<numberOfWeigthHypothesis_; hi++){
      ...
      for (double[] element : nodeCheck_) {
        ...
        constraint[con]= (-deltaN +element[1]);
        con+=1;
      }
    }
 ...
 public double BucklingOmega(double Nxx, double[] G, double[] B) throws JMException{
  ...
  public void EBEsPrintArchTxtReaction(int hi) throws JMException{
    try {
      ...
      // impresion de la matriz de rigidez penalizada
      // extremo ii
      for (double[] element : NodeRestrict_) {
        int no = (int)element[0];
        int ap = (int)element[1];
